...
class BookmarkActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // RecyclerView 설정
        val bookmarkRecyclerView = findViewById<RecyclerView>(R.id.bookmark_recycler_view)
        bookmarkRecyclerView.layoutManager = LinearLayoutManager(this)

        // 데이터 가져오기 (북마크된 항목만 필터링)
        val allActions = getDummyData() // 원래 데이터 가져오기
        val bookmarkedActions = allActions.filterIsInstance<ActionItem.Action>()
            .filter { it.isBookmark }

        // Adapter 연결
        val bookmarkAdapter = BookmarkAdapter(bookmarkedActions)
        bookmarkRecyclerView.adapter = bookmarkAdapter
    }

    // Dummy 데이터 생성 (실제 데이터 소스에서 가져오는 경우로 교체 가능)
    private fun getDummyData(): List<ActionItem> {
        return listOf(
